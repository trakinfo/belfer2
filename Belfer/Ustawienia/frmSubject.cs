...
namespace Belfer
{
    public partial class frmSubject : Form
    {
        ...
        {
            cbSeek.DataSource = new string[] { "Nazwa", "Typ" };
        }
        ...
        private List<OLVColumn> SpecifyCols()
        {
            ...
            Cols.Add(new OLVColumn { Text = "Nazwa", WordWrap = false, AspectName = "Name", MinimumWidth = 200, Width = 300, MaximumWidth = 500, FillsFreeSpace = true, HeaderTextAlign = HorizontalAlignment.Center, ToolTipText = "Nazwa typu szkoły", TextAlign = HorizontalAlignment.Left, UseInitialLetterForGroup = true, HeaderCheckBox = true });
            Cols.Add(new OLVColumn { Text = "Alias", WordWrap = false, AspectName = "Alias", MinimumWidth = 100, Width = 200, FillsFreeSpace = false, HeaderTextAlign = HorizontalAlignment.Center, ToolTipText = "Opis typu szkoły lub pełna nazwa typu", TextAlign = HorizontalAlignment.Left, UseInitialLetterForGroup = true });
            Cols.Add(new OLVColumn { Text = "Typ", WordWrap = false, AspectName = "Type", MinimumWidth = 50, Width = 70, FillsFreeSpace = false, HeaderTextAlign = HorizontalAlignment.Center, ToolTipText = "Opis typu szkoły lub pełna nazwa typu", TextAlign = HorizontalAlignment.Center, UseInitialLetterForGroup = true });
            Cols.Add(new OLVColumn { Text = "Priorytet", WordWrap = false, AspectName = "Priority", MinimumWidth = 70, Width = 50, FillsFreeSpace = false, HeaderTextAlign = HorizontalAlignment.Center, ToolTipText = "Opis typu szkoły lub pełna nazwa typu", TextAlign = HorizontalAlignment.Center, UseInitialLetterForGroup = true });
        ...
        private void olvSzkola_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
            if (e.IsSelected)
            {
                ...
                GetSignature((SubjectModel)((OLVListItem)e.Item).RowObject);
                EnableButton(true);
        ...
        private void GetSignature(SubjectModel SchoolItem)
        {
        ...
        {
            GetData(olvSchoolType);
            SeekHelper.SetListItem<SubjectModel, long>(RecordID, "ID", olvSchoolType);
        }
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Czy na pewno chcesz usunąć wskazane pozycje?", Application.ProductName, MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                try
                {
                    ...
                    foreach (SubjectModel s in olvSchoolType.CheckedObjects) sqlParamWithValue.Add(new Tuple<string, object>("@ID", s.ID));
                    using (var scope = AppSession.TypeContainer.BeginLifetimeScope())
                    {
                        var dbs = scope.Resolve<IDataBaseService>();
        ...
        private void txtSeek_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                case 0://Nazwa
                    olvSchoolType.ModelFilter = new ModelFilter(x => ((SubjectModel)x).Name.StartsWith(txtSeek.Text, StringComparison.CurrentCultureIgnoreCase));
                    break;
                case 1://Typ
                    olvSchoolType.ModelFilter = new ModelFilter(x => ((SubjectModel)x).Priority.ToString().StartsWith(txtSeek.Text, StringComparison.CurrentCultureIgnoreCase));
                    break;
            }
